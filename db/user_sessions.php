...
function logout() {
    ...
    unset($_SESSION['scrambler_author']);
...
function is_author($user_id) {
    if (!isset($_SESSION['scrambler_author'])) {
        if (user_has_access_to_app($user_id)) {
            $_SESSION['scrambler_author'] = true;
        } else {
            $_SESSION['scrambler_author'] = false;
        }
    }
    return $_SESSION['scrambler_author'];
}